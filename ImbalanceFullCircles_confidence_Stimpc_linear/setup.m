...
options.num_trials = 55; % How many trials?

window = false;

%screenNumber = min(Screen('Screens'));%max(Screen('Screens'));
screenNumber = 1;
options.dist = 63; %65; %75; % viewing distance in cm 
...
options.sigma = 75;
options.cutoff = 3.8;%4.2;%3.8; %5.5; %4.2;
%options.radius = factor_radius*options.ppd*scaling_factor; %*(options.width/56.4)/(options.resolution(1)/1600); %4*options.ppd;
options.radius = 4*options.ppd; %3.5*options.ppd; %deg2pix(options, 1.3)*scaling_factor; %deg2pix(options, 1.3)*scaling_factor;%4*options.ppd;
